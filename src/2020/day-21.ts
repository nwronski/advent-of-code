...
import { Attributes, NeighborEntry, NodeEntry } from 'graphology-types';
...
function hasType<T extends Attributes>(type: NodeType) {
  return ({ attributes }: NeighborEntry<T> | NodeEntry<T>) => attributes.type === type;
}
...
function neighborsByType<T extends Attributes>(graph: Graph<T>, node: string, type: NodeType): NeighborEntry<T>[] {
  return [ ...graph.neighborEntries(node) ].filter(hasType(type));
}

/**
 * Day 21 (2020)
 * yarn start --year 2020 21
...
export function solution(contents: string) {
  ...
  const simple = toSimple(graph);

  const allAllergens = nodesByType(simple, 'allergen').sort(({ node: a }, { node: b }) => a.localeCompare(b));
  ...
  const allergenMap = allAllergens.map(({ node }) => ({
    allergen: node,
    recipes: allRecipes
      .map(({ node: recipe }) => simple.hasEdge(recipe, node) ? recipe : null)
    ...
    for (const node of simple.nodes()) {
      if (simple.getNodeAttribute(node, 'type') === 'allergen') {
        ...
        if (possibleIngredients.length === 1) {
          const [ { neighbor: foundIngredient } ] = possibleIngredients;
          for (const { node: allergen } of nodesByType(simple, 'allergen')) {
        ...
        if (neighbors.length === 0) {
          ...
          const [ { neighbor: foundAllergen } ] = neighbors;
          const index = allAllergens.findIndex(({ node: a }) => a === foundAllergen);
